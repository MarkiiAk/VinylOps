...
/** Navegación mes anterior/siguiente del calendario de entregas, vía query params (?anio=&mes=). */
export function MonthNav({ year, month, label }: MonthNavProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Link href={`/calendario?anio=${prev.year}&mes=${prev.month + 1}`}>
      ...
      <Link href={`/calendario?anio=${next.year}&mes=${next.month + 1}`}>
